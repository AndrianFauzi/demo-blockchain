...
const Block = ({ index, data, previousHash, hash, onDataChange }: any) => (
  ...
  <div style={{ border: '1px solid #ccc', borderRadius: '8px', padding: '16px', margin: '8px', width: '250px' }}>
    ...
    <div>
      ...
      <div style={{ fontSize: '12px', wordBreak: 'break-all' }}>{previousHash}</div>
    </div>
    <div>
      <strong>Hash:</strong>
...
const BlockchainDemo = () => {
  ...
  const updateBlockData = (index: any, newData: any) => {
    const updatedBlocks = blocks.map((block, i) => {
      if (i === index) {
        return { ...block, data: newData, hash: Math.random().toString(36).substring(7) };
      }
      if (i > index) {
        console.log(blocks[blocks.length - 1].hash, "previous hash");
        
        return { ...block, previousHash: blocks[blocks.length - 1].hash, hash: Math.random().toString(36).substring(7) };
      }
      return block;
    });
